...
This is deliberate: it does not depend on GitHub release assets, so publishing
works whether this repo is public or private. Do not replace it with a
postinstall downloader — that would couple releases to repo visibility.
...
- Version lives in `npm/package.json` and must match every
  `optionalDependencies` pin. `make build` embeds it via ldflags.
- `RELEASE.md` is the publish checklist. Platform packages publish before the
  main package.
